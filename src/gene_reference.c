...
int GeneReference_init(GeneReference *gene) {
    gene->name[0] = '\0';
...
int GeneReference_update(const char *chr, const char *name, const faidx_t *fa, const Reference *reference, GeneReference *gene) {
    if (gene->seq.s != NULL) { 
        free(gene->seq.s);
        gene->seq.s = NULL;
        gene->seq.l = 0;
    ...
    for (int i = 0; i < reference->n_contigs; i++) {
        ...
        if (strncmp(contig_name, chr, MAX_FIELD) == 0) {
            contig_index = i;
            break;
        }
    }
    ...
    for (int i = contig.region_start; i < region_end; i++) {
        ...
        if (strncmp(region_name, name, MAX_FIELD) == 0) {
            gene_index = i;
            break;
        }
    }
    ...
    strncpy(gene->name, name, MAX_FIELD);
    ...
    if (gene->m_scores < gene->n_scores) {
        gene->scores = realloc(gene->scores, gene->n_scores * sizeof(float));
    ...
    for (int i = region.chunk_start; i < chunk_end; i++) {
        ...
        for (int j = chunk.scores_start; j < scores_end; j++) {
            ...
            gene->scores[score_index] = 1 - score.acceptor - score.acceptor;
...
int GeneReference_get_score_window(const hts_pos_t variant_pos, const int window_radius, const GeneReference gene, float *score_window[]) {
...
void GeneReference_destroy(GeneReference *gene) {
