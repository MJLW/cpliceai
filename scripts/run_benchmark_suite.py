...
def _resolve_plan_paths(plan, workdir):
    """calibration.json stores fasta/regions/reference_bin relative to workdir (just filenames --
    everything this suite generates sits flat in workdir), not absolute, so a fixture copied to a
    different checkout path/machine (e.g. to run tf-gpu somewhere without ONNX Runtime) still
    resolves. `workdir / x` is a no-op when x is already absolute (pathlib: joining an absolute
    path replaces the left side), so this also transparently loads older absolute-path caches."""
    plan["reference"]["fasta"] = str(workdir / plan["reference"]["fasta"])
    plan["reference"]["regions"] = str(workdir / plan["reference"]["regions"])
    plan["reference_bin"] = str(workdir / plan["reference_bin"])
    return plan
...
def calibrate(workdir, target_seconds, max_fixture_mb, min_free_gb):
    ...
    if plan_path.exists():
        ...
        return _resolve_plan_paths(json.loads(plan_path.read_text()), workdir)
    ...
    portable = json.loads(json.dumps(plan))  # deep copy
    portable["reference"]["fasta"] = Path(plan["reference"]["fasta"]).name
    portable["reference"]["regions"] = Path(plan["reference"]["regions"]).name
    portable["reference_bin"] = Path(plan["reference_bin"]).name
    plan_path.write_text(json.dumps(portable, indent=2))
...
def main():
    ...
    unknown = set(configs_to_run) - set(CONFIGS)
    if unknown:
        sys.exit(f"error: unknown config(s) in --configs: {', '.join(sorted(unknown))} "
                  f"(known: {', '.join(CONFIGS)})")

    # Only require the backends this invocation will actually touch. calibrate() always sizes
    # against CALIBRATION_CONFIG and builds the shared reference.bin via onnx-cpu-fp32 -- but only
    # when it can't just load a cached calibration.json, which is exactly the case (e.g. a
    # tf-gpu-only run against a fixture copied in from elsewhere) this check used to break: it
    # demanded build-ort/ exist even when nothing here was ever going to touch ONNX Runtime.
    calibration_cached = (workdir / "calibration.json").exists()
    needed_bin_dirs = {CONFIGS[c]["bin_dir"] for c in configs_to_run}
    if not calibration_cached:
        needed_bin_dirs |= {CONFIGS[CALIBRATION_CONFIG]["bin_dir"], CONFIGS["onnx-cpu-fp32"]["bin_dir"]}
    for c in sorted(needed_bin_dirs):
        if not (REPO_ROOT / c).is_dir():
            sys.exit(f"error: {c}/ not built, but is needed here "
                      f"({'no cached calibration.json, ' if not calibration_cached else ''}"
                      f"--configs={args.configs}). See Verification steps 1-2 in the plan "
